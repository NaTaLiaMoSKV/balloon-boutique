...
import {
  ...
  CounterDisplay,
  CounterWrapper,
} from "components/BalloonSubsection/BalloonSubsection.styled";
import {
  CloseButton,
  ModalButtonsWrapper,
  ModalContent,
                  ...
                  <div className="details-wrapper">
                    <div>
                      <ProductTitle>
                        {item.balloon.title}
                        {item.inscription && (
                          <span> ({truncateText(10, item.inscription)})</span>
                        )}
                      </ProductTitle>
                      {item.balloon.description && (
                        <p className="add-info"> {item.balloon.description}</p>
                      )}
                    </div>

                    <DeleteButton
                      onClick={() => handleRemoveFromCart(item.balloon)}
                    >
                      <img src={deleteImg} alt="delete" />
                    </DeleteButton>
                  </div>
                  {item.balloon.longDescription ===
                    "Додати індивідуальний напис - 100 грн" && (
                    <OpenModalButton
                      onClick={() =>
                        openInscriptionModal(item.balloon.id, item.inscription)
                      }
                    >
                      <img
                        src={item.inscription ? editImg : addImg}
                        alt="редагувати напис"
                      />
                      <span>
                        {item.inscription ? "Змінити надпис" : "Додати надпис"}
                      </span>
                    </OpenModalButton>
                  )}
                  ...
                  <div className="details-wrapper">
                    <ProductPrice>
                      ...
                      {item.inscription && (
                        <span>
                          {" "}
                          <br />+{formatNumber(item.quantity * 100)} ₴
                        </span>
                      )}
                    </ProductPrice>
                    <CounterWrapper>
                      <CounterButton onClick={() => decrement(item.balloon.id)}>
                        -
                      </CounterButton>
                      <CounterDisplay>{counts[item.balloon.id]}</CounterDisplay>
                      <CounterButton onClick={() => increment(item.balloon.id)}>
                        +
                      </CounterButton>
                    </CounterWrapper>
                  </div>
                  {item.balloon.price === 0 && (
                    <p className="add-info">Менеджер повідомить вам про ціну</p>
                  )}
                </MobileCartItemWrapper>
              ))}
                  ...
                  <TableRow key={item.balloon.id}>
                    <TableColumn className="photo-column">
                      <ProductImage
                        src={item.balloon.image}
                        alt={item.balloon.title}
                        onClick={() => handleCardClick(item.balloon.image)}
                      />
                    </TableColumn>
                    <TableColumn className="info-column">
                      <div className="title-wrapper">
                        <ProductTitle>
                          {item.balloon.title}
                          {item.inscription && (
                            <span> ({truncateText(22, item.inscription)})</span>
                          )}
                        </ProductTitle>
                        {item.balloon.description && (
                          <p>{item.balloon.description}</p>
                        )}
                      </div>
                      {item.balloon.longDescription ===
                        "Додати індивідуальний напис - 100 грн" && (
                    ...
                    <TableColumn className="action-column">
                      ...
                      <div className="counter-wrapper">
                        <CounterWrapper>
                          <CounterButton
                            onClick={() => decrement(item.balloon.id)}
                          >
                            -
                          </CounterButton>
                          <CounterDisplay>
                            {counts[item.balloon.id]}
                          </CounterDisplay>
                          <CounterButton
                            onClick={() => increment(item.balloon.id)}
                          >
                            +
                          </CounterButton>
                        </CounterWrapper>
        ...
        {modalOpen && (
          <ModalOverlay>
            <ModalContent>
              ...
              <ModalButtonsWrapper>
                <OpenModalButton
                  disabled={currentInscription.trim() === ""}
